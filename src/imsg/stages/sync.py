...
SegmentFn = Callable[[psycopg.Connection, Config], object]
...
`functools.partial` before passing the result here."""

EmbedFn = Callable[[psycopg.Connection, Config], object]
"""S6's entry point (not yet built anywhere in this repo as of this
build) — same binding convention as `SegmentFn`."""
...
def run_sync(
    *,
    ...
    if snapshot_override is not None:
        ...
        try:
            snapshot_result = run_snapshot_fn(live_chat_db=resolved_chat_db, data_root=config.paths.data_root)
    ...
    try:
        identity_result = run_identity_fn(conn=conn, config=config, contacts_importer=contacts_importer)
    except ImsgError as exc:
...
def run_sync_all_sources(
    ...
    **kwargs: Any,
    ...
    time `run_sync`'s injectable seams change.
